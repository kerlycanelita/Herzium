...
That hash identifies the jar this evidence was captured against, before the
audit fixes in `docs/audits/AUDIT-1.9.3.md` were applied, and is kept for exactly that
reason: it says which build the screenshots show. It is not the hash of the
current jar. The checksums of what the build produces now are generated by the
`herziumChecksums` Gradle task into `docs/checksums.txt`.
